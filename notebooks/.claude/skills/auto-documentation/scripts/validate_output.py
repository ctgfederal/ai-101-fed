#!/usr/bin/env python3
"""
validate_output.py — validate a written auto-doc file end-to-end.

Checks:
  - Frontmatter parses
  - Frontmatter has every required field, non-empty
  - category is one of the four canonical values
  - date is YYYY-MM-DD and not in the future
  - tags is a 2-8 item list of valid tag tokens
  - Body has the four required sections in order: Description, Why, Examples, Related

Usage:
  python validate_output.py --file path/to/doc.md
...
REQUIRED_FIELDS = ["title", "category", "date", "tags", "scope", "source"]
REQUIRED_SECTIONS = ["Description", "Why", "Examples", "Related"]
...
def validate_frontmatter(data: dict) -> List[str]:
    ...
    if cat and cat not in CANONICAL_CATEGORIES:
        errors.append(f"unknown category: {cat!r} (allowed: {sorted(CANONICAL_CATEGORIES)})")
...
def validate_body(body: str) -> List[str]:
    ...
    if indices and indices != sorted(indices):
        errors.append(f"body sections out of order; expected: {REQUIRED_SECTIONS}, got: {found}")
    return errors
...
def main() -> int:
    ...
    errors = validate_frontmatter(data) + validate_body(body)
    if errors:
        for err in errors:
            log.error(err)
        return 1
